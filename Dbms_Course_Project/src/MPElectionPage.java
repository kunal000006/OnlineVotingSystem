...
public class MPElectionPage extends JFrame {
    ...
    private Set<Integer> votedElectionIds = new HashSet<>(); // Store voted election IDs

    public MPElectionPage(String region, int voterId) {
        currentRegion = region;
        this.voterId = voterId;
        setTitle("MP Election");
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                boolean isRunning = isElectionRunning(region, "MP");
                if (isRunning) {
                    java.util.List<String[]> candidatesData = getCandidates(region, "MP");
                ...
                updateRemainingTimeLabel(region, "MP");
            }
        }, 0, 10000);

        // Initialize the votedElectionIds set with the election IDs for which the voter has already voted
    ...
    private void voteForSelectedCandidate() {
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            if (selected) {
                ...
                int electionId = getElectionId(currentRegion, "MP"); // Use the stored current region
                ...
                vote(candidateName, partyName, electionId);
    ...
    private void vote(String candidateName, String partyName, int electionId) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            // Add the current election ID to the set of voted election IDs
    ...
    private void insertVoteDetails(String candidateName, String partyName, int electionId) {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            String getCandidateIdSQL = "SELECT candidate_id FROM candidates WHERE name = ? AND party = ? AND region = ? AND position='MP'";
    ...
    private boolean isElectionValid(int electionId, int candidateId) {
        try (Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            String sql = "SELECT COUNT(*) FROM elections WHERE election_id = ? AND region = ? AND position = 'MP'";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    private void navigateToGiveYourVotePage(String region, int voterId) {
        // Create an instance of the GiveYourVotePage and show it
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                new MPElectionPage("Your Region", 12345).setVisible(true); // Replace with actual region and voter ID
            }
        });
    }
}
